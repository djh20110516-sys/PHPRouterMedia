<?php
	...
	$G_SQLITE_TABLES = array(
        //'table' => 'CREATE TABLE',
        'config' => "CREATE TABLE 'config' ('key' TEXT PRIMARY KEY NOT NULL, 'value' TEXT)",
	...
	function sqlite_config_get( $key, $default = '' ){
        $result = $default;
        if( ( $dbhandle = sqlite_init() ) != FALSE ){
            $sql = "SELECT value FROM config WHERE key = '" . $dbhandle->escapeString( $key ) . "'";
	...
	function sqlite_config_set( $key, $value ){
        $result = FALSE;
        if( ( $dbhandle = sqlite_init() ) != FALSE ){
            $sql = "REPLACE INTO config (key, value) VALUES ('" . $dbhandle->escapeString( $key ) . "', '" . $dbhandle->escapeString( $value ) . "')";
	...
	function sqlite_media_getdata_identify_orderer( $search = '', $limit = 100 ){
		...
		// First: unidentified media (idmediainfo = 0)
		if( ( $data = sqlite_media_getdata_identify_new( $search, $limit ) ) != FALSE
		&& is_array( $data )
		){
	...
	function sqlite_media_getdata_identify_new( $search = '', $limit = 100 ){
		//Vars
		$result = FALSE;
		...
		if( ( $dbhandle = sqlite_init() ) != FALSE ){
			...
			$sql .= ' WHERE media.idmediainfo = 0 OR media.idmediainfo IS NULL';
			if( strlen( $search ) > 0 ){
