<?php
	...
	//Get downloads path (from DB config or default)
	function get_downloads_path(){
        global $G_DB;
        $default = defined( 'PPATH_DOWNLOADS' ) ? PPATH_DOWNLOADS : '/home/hao/download/movie';
        if( $G_DB != FALSE || ( $G_DB = @new SQLite3( PPATH_CACHE . DS . 'data.db', SQLITE3_OPEN_READONLY ) ) != FALSE ){
            $sql = "SELECT value FROM config WHERE key = 'ppath_downloads'";
            $res = $G_DB->query( $sql );
            if( $res ){
                $row = $res->fetchArray( SQLITE3_ASSOC );
                if( $row && array_key_exists( 'value', $row ) && strlen( $row[ 'value' ] ) > 0 ){
                    if( is_resource( $G_DB ) || $G_DB instanceof SQLite3 ){
                        $G_DB->close();
                    }
                    $G_DB = FALSE;
                    return $row[ 'value' ];
